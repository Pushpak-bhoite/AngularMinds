...
export default function () {
    ...
    function onEditing(event, id){
        console.log(event.target.value, id)
        let {name, value} = event.target;
        setArrData( prev => prev?.map(e => id === e.id ? {...e,[name]:value} :  e  ))
    }
                        ...
                        <div className="white-card">
                            ...
                            <div id="showData">
                                <div className="ul-outer">
                                    <ul id="ulShow">
                                        {
                                            arrData?.map((data) => <List key={data.id} id={data.id} editTask={() => editTask(data.id)} handleChange={handleChange} dataVal={data.tName} editing={data.editing} onEditing={onEditing}/>)
                                        }
                                    </ul>
                                </div>
                            </div>
